...
- `app/decisions/builtin_rules.py`：内置高置信交通识别规则（地铁_/单车/骑行/公交/机票/火车票/XX出行 → 出行交通），新导入自动入账，可对存量 unmatched 一次性应用。
- `app/decisions/rule_sweep.py`：历史批量确认规则清扫与 unmatched 规则自动筛选。
