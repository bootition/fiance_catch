...
| 重构阶段 1 | 🔄 审查未通过：新 schema（8 表）、仓储层和 pytest 基座完成；迁移尚未实现已确认的“备份后重置为干净新模型”，且备份一致性不足 | `docs/reports/01_phase1_review_2026-08-01.md` |
| 当前产品面（未重构前） | ✅ 记账 `/` + 复盘 `/review` + 清理 `/cleanup`；导入 UI 已移除，仅保留历史元数据 | `docs/decisions/02_architecture.md` |
| 测试基线 | ✅ 直接 `pytest` 112 项通过（77 旧 + 35 新，含 schema/仓储/迁移）；但现有迁移测试固定了与规格冲突的旧表保留行为 | `docs/reports/01_phase1_review_2026-08-01.md` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 1 有两个阻塞缺陷：活动库未在备份后清除旧业务表/数据；备份使用 `shutil.copy2()`，无法保证 SQLite 一致性快照。详见 `reports/01_phase1_review_2026-08-01.md`。
...
- 账单驱动重构：阶段 1 待修复迁移重置与备份一致性缺陷；修复并重新审查通过后，才能进入阶段 2（账单解析与批次导入）
