...
6. 真实账单导入已完成并验收（2026-08-14 复核通过，见 `reports/23_*`）；当前剩余 **850 条**待确认（未匹配 607、人际转账 108、中性资金流 66、退款 64、提现 5）。规则持续由用户操作 AI 写入并自动筛选（备份 `ledger.sqlite-20260827-234749.bak`）。处理完成前，正式统计以 `/inbox` 处理进度为准，不宣称"账已对平/已正式发布"。
...
- 用户验收：真实账单已导入并通过验收复核；当前等待用户在 `/inbox` 处理 **850 条**待确认（分类确认、退款关联、提现用途、人际/中性资金流定性），并处理规则 #2 的 17 笔退回重分类，随后核对月度统计并生成用户验收报告。
- 十一轮用户反馈修复已完成并写入 `docs/reports/28_*`~`38_*`；会话计划与发现记录在 `.planning/2026-08-27-user-feedback*/`（不入 git）。
