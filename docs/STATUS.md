...
| 重构阶段 3 | ✅ 完成：决策引擎（退款/提现/人际转账待确认、明确调拨自动入账、active 规则自动入账、观察期规则预填待确认、未命中待确认+副业收入建议）；相似项分组与批量确认（≥2 条建议创建观察规则）；规则提升；`process_batch` 单事务幂等 | `docs/decisions/01_refactor_spec.md` §2/§3.3/§3.5/§5/§7.3、`app/decisions/` |
| 当前产品面 | ✅ `/` 为 v2 迁移状态维护页；旧页面路由已下线，阶段 5 重建 | `app/routers/status.py` |
| 测试基线 | ✅ 直接 `pytest` 155 项通过（新增决策引擎/分组确认 15 项）；真实账单冒烟：支付宝 545 调拨+1428 待确认、微信 17 调拨+332 待确认，退款 64 笔全部入队 | `tests/test_decisions.py` |
...
- 账单驱动重构：阶段 3（规则、观察期、待确认队列、批量确认）已完成；下一步阶段 4（退款候选匹配、人工关联、跨期回写、安全批次撤销），待用户安排
