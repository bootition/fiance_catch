...
| 重构阶段 2 | ✅ 审查通过：导入改为单事务原子操作（创建批次+写入全部流水+更新计数，`BEGIN IMMEDIATE`，失败完整回滚可安全重传）；空单号计 invalid 不入库；零金额成功行保留 | `docs/decisions/01_refactor_spec.md` §5/§7.2、`docs/reports/03_phase2_review_2026-08-01.md` + 本会话修复 |
| 当前产品面 | ✅ `/` 为 v2 迁移状态维护页；旧页面路由已下线，阶段 5 重建 | `app/routers/status.py` |
| 测试基线 | ✅ 直接 `pytest` 140 项通过（新增中途失败/计数失败回滚、失败后重传、空单号、零金额 5 项；真实账单冒烟：支付宝 2017 行/微信 349 行与账单头部计数完全吻合） | `tests/test_import_service.py` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 3~6 未实施：规则与观察期、待确认队列、退款候选匹配、页面重建、端到端测试（见 `decisions/01` §7）。
...
- 账单驱动重构：阶段 2 审查通过（2026-08-01，`docs/reports/03_phase2_review_2026-08-01.md`）；下一步阶段 3（规则、观察期、待确认队列、批量确认），待用户安排
