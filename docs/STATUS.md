...
| 重构阶段 5 | ✅ 完成：页面重建——概览 `/`（本月指标/日常环比/分类排行/待办入口）、导入 `/imports/new`（上传+结果计数）、批次 `/imports`（列表+安全撤销）、待确认 `/inbox`（高风险区/分类区分组批量确认）、流水 `/transactions`（筛选+简易补账+编辑删除）、规则 `/rules`（创建/启用/停用/提升/命中历史） | `docs/decisions/01_refactor_spec.md` §3/§7.5、`app/routers/`、`app/stats.py` |
| 当前产品面 | ✅ 全新 v2 页面已上线（旧路由已下线）；维护页与 status 路由已移除 | `app/main.py`、`templates/base.html` |
| 测试基线 | ✅ 直接 `pytest` 208 项通过（新增页面级测试 12 项：概览指标/导入流程/重复去重/高风险与分组/批量确认/补账/筛选/删除/规则操作/批次撤销） | `tests/test_pages.py` |
...
- 账单驱动重构：阶段 5（页面重建 + 简易补账）已完成；下一步阶段 6（端到端测试：匿名化固定样本），待用户安排
