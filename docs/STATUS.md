...
| 重构阶段 6 | ✅ 完成：匿名化固定样本（tests/samples/ 真实格式支付宝 CSV + 微信 XLSX）+ 9 项端到端测试覆盖全部验收条件（去重/规则观察期与自动入账/高风险不污染统计/跨期退款净成本/批量确认建规则/撤销阻塞/首页口径/不存原文件/全流程） | `docs/decisions/01_refactor_spec.md` §7.6/§8、`tests/test_e2e.py` |
| 当前产品面 | ✅ 全新 v2 页面已上线（旧路由已下线）；维护页与 status 路由已移除 | `app/main.py`、`templates/base.html` |
| 测试基线 | ✅ 直接 `pytest` 228 项通过（含 9 项端到端验收测试） | `tests/test_e2e.py` |
...
- 账单驱动重构：全部六阶段实施完成（阶段 6 端到端测试 2026-08-01 完成）；待各阶段最终审查通过后交付验收
