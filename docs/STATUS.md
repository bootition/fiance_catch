...
| 重构阶段 5 | 🔄 红队审查未通过：规则页面绕过观察期；概览多笔部分退款统计错误；编辑已退款消费可产生负净额 | `docs/reports/12_phase5_red_team_review_2026-08-01.md` |
| 当前产品面 | ✅ 全新 v2 页面已上线（旧路由已下线）；维护页与 status 路由已移除 | `app/main.py`、`templates/base.html` |
| 测试基线 | ⚠️ 直接 `pytest` 208 项通过，但未覆盖规则状态机、部分退款概览聚合和已退款记录编辑约束 | `docs/reports/12_phase5_red_team_review_2026-08-01.md` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 5 当前阻塞：规则观察期可绕过、部分退款概览统计错误、已退款消费可被编辑为负净额。详见 `reports/12_phase5_red_team_review_2026-08-01.md`。
...
- 账单驱动重构：阶段 5 待修复规则状态机、退款统计聚合和已退款记录编辑约束；重新红队审查通过后才能进入阶段 6
