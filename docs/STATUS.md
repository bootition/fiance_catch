...
| 重构阶段 6 | ✅ 修复完成：账单行交易时间解析规范化（`YYYY-MM-DD HH:MM:SS`），空/非法/不存在日历日期抛 ValueError 整批原子回滚；来源流水与账本日期全部公开写入口统一校验 | `docs/decisions/01_refactor_spec.md` §5/§7.2/§7.6/§8、`docs/reports/17_phase6_second_red_team_review_2026-08-01.md` + 本会话修复 |
| 当前产品面 | ✅ 全新 v2 页面已上线（旧路由已下线）；维护页与 status 路由已移除 | `app/main.py`、`templates/base.html` |
| 测试基线 | ✅ 直接 `pytest` 248 项通过（新增 10 项：支付宝/微信无效日期、无效时间、不存在日历日、合法闰日；页面级端到端 2 项坏日期上传无批次/无来源残留） | `tests/test_import_service.py`、`tests/test_e2e.py` |
...
- 账单驱动重构：阶段 6 二次红队复审修复已完成（账单行时间解析规范化 + 全写入口日期校验 + 10 项新测试）；全部六阶段实施完成，待最终红队复审交付验收
