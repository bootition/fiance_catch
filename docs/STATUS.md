...
| 用户反馈第二十轮修复 | ✅ 通过：亲情卡方向纠正为消费·日常缴费；规则方向目标一致性校验；提现界面显示哈利布朗等调拨分类 | `docs/reports/47_user_feedback_round20_family_card_validation_withdrawal_ui_2026-08-28.md` |
...
6. 真实账单导入已完成并验收（2026-08-14 复核通过，见 `reports/23_*`）；当前剩余 **344 条**待确认（未匹配 247、人际转账 44、退款 46、其他中性 2、提现 5）。退款严格等额且不限时间窗口；亲情卡方向已纠正；余利宝/生活费规则已执行（备份 `ledger.sqlite-20260828-212040.bak`）。处理完成前，正式统计以 `/inbox` 处理进度为准，不宣称"账已对平/已正式发布"。
