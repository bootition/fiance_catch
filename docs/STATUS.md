...
| 重构阶段 5 | ✅ 红队审查通过：P1-1 规则状态机服务层强制（observing 不可直跳 active，必须 promote）；P1-2 概览退款子查询预聚合 + 聚合内扣减（多笔部分退款/多笔消费混合正确）；P1-3 仓储层约束已退款记录（金额 ≥ 已关联退款总额、类型不可变） | `docs/decisions/01_refactor_spec.md` §2.2/§2.3/§3.1/§3.5/§6/§7.5、`docs/reports/12_phase5_red_team_review_2026-08-01.md` + 本会话修复 |
| 当前产品面 | ✅ 全新 v2 页面已上线（旧路由已下线）；维护页与 status 路由已移除 | `app/main.py`、`templates/base.html` |
| 测试基线 | ✅ 直接 `pytest` 217 项通过（新增红队回归 9 项：观察期直跳拒绝+观察语义保持、部分退款聚合/排行/跨期、编辑负净额/类型变更拒绝、非退款记录编辑放行、页面路由层面防御） | `tests/test_red_team_phase5.py` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 6 未实施：端到端测试（匿名化固定样本覆盖重复导入、退款、调拨、批量确认、撤销阻塞和统计，见 `decisions/01` §7.6/§8）。
...
- 账单驱动重构：阶段 5 红队审查通过（2026-08-01，`docs/reports/12_phase5_red_team_review_2026-08-01.md`）；下一步阶段 6（端到端测试：匿名化固定样本），待用户安排
