...
| 重构阶段 2 | ✅ 完成：支付宝 CSV（GB18030）与微信 XLSX 解析、标准化、状态处理（成功/跳过/退款）、同平台来源去重、导入批次与计数 | `docs/decisions/01_refactor_spec.md` §5/§7.2、`app/importing/` |
| 当前产品面 | ✅ `/` 为 v2 迁移状态维护页；旧页面路由已下线，阶段 5 重建 | `app/routers/status.py` |
| 测试基线 | ✅ 直接 `pytest` 135 项通过（新增解析/导入 17 项；真实账单冒烟：支付宝 2017 行/微信 349 行与账单头部计数完全吻合） | `tests/test_parsers.py`、`tests/test_import_service.py` |
...
- 账单驱动重构：阶段 2（账单解析与批次导入）已完成；下一步阶段 3（规则、观察期、待确认队列、批量确认），待用户安排
