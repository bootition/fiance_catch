...
| 重构阶段 1 | ✅ 复审通过：P1/P2 修复已验证；生产 `init_db()` 后 `/` 返回维护页（200），旧路由已禁用（404） | `docs/reports/02_phase1_fix_review_2026-08-01.md`（approved）+ 本会话修复 |
| 当前产品面 | ✅ `/` 为 v2 迁移状态维护页（新表行数、迁移时间、备份列表）；旧页面（记账/复盘/清理）路由已下线，阶段 5 重建 | `app/routers/status.py`、`templates/maintenance.html` |
| 测试基线 | ✅ 直接 `pytest` 118 项通过（新增生产 `init_db()` 端到端路由测试：`/` 200、旧路由 404、重复启动幂等） | `tests/test_v2_routes.py` |
...
- 账单驱动重构：阶段 1 复审通过（2026-08-01，`docs/reports/02_phase1_fix_review_2026-08-01.md`）；下一步阶段 2（账单解析与批次导入），待用户安排
