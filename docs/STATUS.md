...
6. 真实账单导入已完成并验收（2026-08-14 复核通过，见 `reports/23_*`）；当前剩余 **402 条**待确认（未匹配 247、人际转账 92、退款 47、其他中性 11、提现 5）。退款匹配取消时间窗口，同日/同平台/同金额优先，覆盖候选支持退款大于原消费（备份 `ledger.sqlite-20260828-012000.bak`）。处理完成前，正式统计以 `/inbox` 处理进度为准，不宣称"账已对平/已正式发布"。
...
- 用户验收：真实账单已导入并通过验收复核；当前等待用户在 `/inbox` 处理 **402 条**待确认（分类确认、退款关联、提现用途、人际/中性资金流定性），并处理规则 #2 的 17 笔退回重分类，随后核对月度统计并生成用户验收报告。
- 十七轮用户反馈修复已完成并写入 `docs/reports/28_*`~`44_*`；会话计划与发现记录在 `.planning/2026-08-27-user-feedback*/`（不入 git）。
