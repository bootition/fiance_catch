...
| 测试基线（第八轮） | ✅ 直接 `pytest` **298 项通过**（296 基线 + raw_type 规则 1 项） | `docs/reports/35_*` |
...
| 用户反馈第十轮修复 | ✅ 通过：统计副业收入 155 笔/22 个商品；每个商品写入 active 商品规则，规则表增至 39 条 | `docs/reports/37_user_feedback_round10_income_product_rules_2026-08-27.md` |
| 用户反馈第十一轮修复 | ✅ 通过：新增衣物用品与调拨专用分类；写入闲鱼成本/学习/校园卡/衣物等 23 条规则并自动筛选 291 笔；说明转账与提现位置 | `docs/reports/38_user_feedback_round11_categories_rules_transfer_evidence_2026-08-27.md` |
| 用户反馈第十二轮修复 | ✅ 通过：话费/起点币/美团形态/基金哈利布朗规则；分类下拉按类型过滤；退款候选显示账本ID与单号；说明拼多多账单无商品信息 | `docs/reports/39_user_feedback_round12_rules_and_refund_clarity_2026-08-28.md` |
| 用户反馈第十三轮修复 | ✅ 通过：新增被坑智商税/突发消费/锻炼花费；京东规则；高风险区统一分类框；单挑啊 11 笔归日常缴费 | `docs/reports/40_user_feedback_round13_categories_jd_highrisk_person_2026-08-28.md` |
| 用户反馈第十四轮修复 | ✅ 通过：唯一候选退款自动冲销 7 笔；多候选/无候选保留人工或等分类后再匹配；退款不虚增财产 | `docs/reports/41_user_feedback_round14_refund_auto_offset_2026-08-28.md` |
| 用户反馈第十五轮修复 | ✅ 通过：京东微信订单 21 笔归三餐；花呗还款丢弃 6 笔；余额宝利息 447 笔改为其他收入；基金买卖归哈利布朗；高风险区表格化紧凑 | `docs/reports/42_user_feedback_round15_jd_interest_huabei_fund_ui_2026-08-28.md` |
| 用户反馈第十六轮修复 | ✅ 通过：退款取消时间窗口并按同日/同平台/同金额优先；翻页位置修复；基金宽规则回退并给出纯债C等建议 | `docs/reports/43_user_feedback_round16_refund_matching_scroll_fund_rollback_2026-08-28.md` |
| 用户反馈第十七轮修复 | ✅ 通过：#690 与 Figma 退款自动冲销；覆盖候选支持退款大于原消费；高风险区横向布局 | `docs/reports/44_user_feedback_round17_refund_figma_withdrawal_ui_2026-08-28.md` |
| 用户反馈第十八轮修复 | ✅ 通过：Figma 金额不同回退；退款候选默认折叠并排除平台/超额差异；修复微信部分退款原消费误判，孔夫子退款候选可见 | `docs/reports/45_user_feedback_round18_figma_revert_collapse_kong_2026-08-28.md` |
| 用户反馈第十九轮修复 | ✅ 通过：退款严格等额；亲情卡16笔归副业收入；余利宝/网商银行9笔忽略；新增生活费并处理9笔；退款无候选原因诊断 | `docs/reports/46_user_feedback_round19_rules_and_refund_diagnostics_2026-08-28.md` |
| 用户反馈第二十轮修复 | ✅ 通过：亲情卡方向纠正为消费·副业成本；规则方向目标一致性校验；提现界面显示哈利布朗等调拨分类 | `docs/reports/47_user_feedback_round20_family_card_validation_withdrawal_ui_2026-08-28.md`（superseded） |
| PDD 订单同步 v8 | ✅ 上线验收通过：schema v8、`/pdd` 导入与匹配、富化展示、退款订单链接；350 订单导入、211/217 支出高置信富化、25/26 退款订单唯一候选；两笔错误退款链接回滚、5 组微信全额退款冲抵；`pytest` 302 通过 | `docs/reports/48_pdd_sync_v8_2026-08-29.md`、`docs/decisions/04_pdd_order_sync_spec.md` |
...
6. 真实账单导入已完成并验收（2026-08-14 复核通过，见 `reports/23_*`）；当前剩余 **336 条**待确认（PDD v8 冲抵处理净减 8 条）。退款严格等额且不限时间窗口；PDD 退款订单链接已启用（25 条）。处理完成前，正式统计以 `/inbox` 处理进度为准，不宣称"账已对平/已正式发布"。
...
13. PDD 订单同步/回填长期功能已实施（v8 MVP）：350 订单导入、216 active/1 manual_review 富化、25 退款订单链接、原始账单字段已恢复。剩余 6 笔支出无订单（1 medium + 5 none）和 16 笔退款待处理（多数缺少原支付流水），保持人工。后续待办：冲抵占位分类 5 笔需用户改真实分类；采集器 CLI 与复核页可按需深化。

## 进行中的工作

- 用户验收：真实账单已导入并通过验收复核；当前等待用户在 `/inbox` 处理 **344 条**待确认（分类确认、退款关联、提现用途、人际/中性资金流定性），并处理规则 #2 的 17 笔退回重分类，随后核对月度统计并生成用户验收报告。
- 十九轮用户反馈修复已完成并写入 `docs/reports/28_*`~`46_*`；会话计划与发现记录在 `.planning/2026-08-27-user-feedback*/`（不入 git）。
- PDD 订单同步/回填长期功能：规格已批准并完成 MVP 上线（`docs/reports/48_pdd_sync_v8_2026-08-29.md`）；后续按 `.planning/2026-08-28-pdd-long-term/task_plan.md` 继续深化采集器 CLI、复核页与退款成对归并。
...
| `docs/decisions/04_pdd_order_sync_spec.md` | PDD 订单同步/富化/退款链接规格 | approved，2026-08-29 |
