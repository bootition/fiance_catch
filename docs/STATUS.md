...
| 重构阶段 6 | 🔄 红队审查未通过：端到端测试主要绕过页面入口；非法日期补账会先写坏记录再返回 500，非法平台/规则字段也返回 500 | `docs/reports/15_phase6_red_team_review_2026-08-01.md` |
| 当前产品面 | ✅ 全新 v2 页面已上线（旧路由已下线）；维护页与 status 路由已移除 | `app/main.py`、`templates/base.html` |
| 测试基线 | ⚠️ 直接 `pytest` 228 项通过，但缺少页面表单篡改的端到端覆盖 | `docs/reports/15_phase6_red_team_review_2026-08-01.md` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 6 当前阻塞：非法日期补账会落库后 500，非法平台/规则字段返回 500；端到端测试未覆盖页面输入篡改。详见 `reports/15_phase6_red_team_review_2026-08-01.md`。
...
- 账单驱动重构：阶段 6 待修复页面入口校验与端到端表单篡改覆盖；重新红队审查通过后交付验收
