...
- **最后更新**：2026-08-28
- **更新人**：用户反馈第二十轮修复；PDD 订单回填一次性实施 + 长期功能规格草案
...
| 用户反馈第二十轮修复 | ✅ 通过：亲情卡方向纠正为消费·副业成本；规则方向目标一致性校验；提现界面显示哈利布朗等调拨分类 | `docs/reports/47_user_feedback_round20_family_card_validation_withdrawal_ui_2026-08-28.md` |
...
6. 真实账单导入已完成并验收（2026-08-14 复核通过，见 `reports/23_*`）；当前剩余 **344 条**待确认（未匹配 247、人际转账 44、退款 46、其他中性 2、提现 5）。退款严格等额且不限时间窗口；亲情卡方向已纠正；余利宝/生活费规则已执行（备份 `ledger.sqlite-20260828-212818.bak`）。处理完成前，正式统计以 `/inbox` 处理进度为准，不宣称"账已对平/已正式发布"。
...
13. **PDD 订单同步/回填长期功能未实施**：规格草案 `docs/decisions/04_pdd_order_sync_spec.md`（status: draft，2026-08-28）待用户确认 D1-D5；任务清单见 `.planning/2026-08-28-pdd-long-term/task_plan.md`（不入 git）。2026-08-28 已完成一次性抓取与回填：350 笔订单、211/217 笔支出高置信回填（可回滚），6 笔待人工复核。
...
- PDD 订单同步/回填长期功能：规格草案已落 `docs/decisions/04_pdd_order_sync_spec.md`（draft，待批准），实施清单在 `.planning/2026-08-28-pdd-long-term/task_plan.md`；未批准前不开始 Phase 1-7。
