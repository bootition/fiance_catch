...
| 重构阶段 5 | 🔄 修复红队复审未通过：原三项 P1 已修复，但页面删除已关联退款的消费会触发未处理 SQLite 外键错误并返回 HTTP 500 | `docs/reports/13_phase5_fix_red_team_review_2026-08-01.md` |
| 当前产品面 | ✅ 全新 v2 页面已上线（旧路由已下线）；维护页与 status 路由已移除 | `app/main.py`、`templates/base.html` |
| 测试基线 | ⚠️ 直接 `pytest` 217 项通过，但未覆盖页面删除已关联退款记录的外键冲突 | `docs/reports/13_phase5_fix_red_team_review_2026-08-01.md` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 5 当前阻塞：删除已关联退款消费的正常页面操作返回 HTTP 500，需将外键冲突转为用户可处理提示。详见 `reports/13_phase5_fix_red_team_review_2026-08-01.md`。
2. 重构阶段 6 未实施：端到端测试（匿名化固定样本覆盖重复导入、退款、调拨、批量确认、撤销阻塞和统计，见 `decisions/01` §7.6/§8）。
...
- 账单驱动重构：阶段 5 待修复已关联退款记录删除的外键冲突处理；重新红队审查通过后才能进入阶段 6
