...
| 重构阶段 6 | ✅ 红队审查通过：P1 补账/编辑路由写前验证 ISO 日期（非法日期不落库、不 500）；P2 导入平台枚举校验 + 规则字段枚举/完整性异常捕获；新增 8 项页面表单篡改端到端测试 | `docs/decisions/01_refactor_spec.md` §7.6/§8、`docs/reports/15_phase6_red_team_review_2026-08-01.md` + 本会话修复 |
| 当前产品面 | ✅ 全新 v2 页面已上线（旧路由已下线）；维护页与 status 路由已移除 | `app/main.py`、`templates/base.html` |
| 测试基线 | ✅ 直接 `pytest` 236 项通过（新增页面级端到端 8 项：非法日期/日历日期/类型/金额/平台/规则字段/规则目标类型/已退款删除，均验证失败不写库且不 500） | `tests/test_e2e.py` |
...
- 账单驱动重构：阶段 6 红队审查通过（2026-08-01，`docs/reports/15_phase6_red_team_review_2026-08-01.md`）；全部六阶段实施完成，待最终交付验收
