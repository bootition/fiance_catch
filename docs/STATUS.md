...
| 重构阶段 3 | 🔄 红队审查未通过：高风险待办可经批量确认绕过用途/退款关联；空规则可匹配全部交易；批次重跑会将仍待确认数清零 | `docs/reports/05_phase3_red_team_review_2026-08-01.md` |
| 当前产品面 | ✅ `/` 为 v2 迁移状态维护页；旧页面路由已下线，阶段 5 重建 | `app/routers/status.py` |
| 测试基线 | ⚠️ 直接 `pytest` 155 项通过，但未覆盖高风险批量确认、空规则全量匹配和幂等重跑计数 | `docs/reports/05_phase3_red_team_review_2026-08-01.md` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 3 当前阻塞：批量确认可绕过提现/退款/人际资金处理，空规则可自动错分全部交易，重跑批次会清零仍待确认数。详见 `reports/05_phase3_red_team_review_2026-08-01.md`。
...
4. 阶段 3 边界说明：退款行已入队（refund_pending）但匹配原消费属阶段 4；页面（概览/待确认/规则/批次）属阶段 5。
5. 阶段 5 若需在导入历史展示无来源单号异常数，应持久化 `invalid_count`，或明确将其归入 `skipped_count`；当前仅由 `ImportResult` 返回。

## 进行中的工作

- 账单驱动重构：阶段 3 待修复红队发现的高风险待办隔离、规则模式校验和批次计数幂等；重新审查通过后才能进入阶段 4
