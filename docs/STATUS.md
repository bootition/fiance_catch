...
| 重构阶段 4 | 🔄 修复红队复审未通过：新退款服务约束有效，但旧公开 `ledger_repo.link_refund()` 可绕过来源真实性、待办、超额和审计约束 | `docs/reports/10_phase4_fix_red_team_review_2026-08-01.md` |
| 当前产品面 | ✅ `/` 为 v2 迁移状态维护页；旧页面路由已下线，阶段 5 重建 | `app/routers/status.py` |
| 测试基线 | ⚠️ 直接 `pytest` 194 项通过，但未覆盖旧 `ledger_repo.link_refund()` 对业务约束的绕过 | `docs/reports/10_phase4_fix_red_team_review_2026-08-01.md` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 4 当前阻塞：旧公开退款写入口可用普通收入虚减消费，绕过新退款服务的所有业务校验。详见 `reports/10_phase4_fix_red_team_review_2026-08-01.md`。
2. 重构阶段 5~6 未实施：页面重建（概览/导入/待确认/流水/规则/批次）、简易补账、端到端测试（见 `decisions/01` §7）。
...
- 账单驱动重构：阶段 4 待封闭旧退款写入口；重新红队审查通过后才能进入阶段 5
