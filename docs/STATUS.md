...
| 重构阶段 4 | 🔄 红队审查未通过：退款关联接受非退款来源且同一退款可关联多个消费，均可重复虚减消费统计 | `docs/reports/09_phase4_red_team_review_2026-08-01.md` |
| 当前产品面 | ✅ `/` 为 v2 迁移状态维护页；旧页面路由已下线，阶段 5 重建 | `app/routers/status.py` |
| 测试基线 | ⚠️ 直接 `pytest` 186 项通过，但未覆盖非退款来源关联、同一退款多重关联和已全额退款候选过滤 | `docs/reports/09_phase4_red_team_review_2026-08-01.md` |

（✅=已通过 🔄=进行中 ⏳=待执行）

## 已知剩余缺口（诚实披露）

1. 重构阶段 4 当前阻塞：非退款来源可冲减消费，同一退款可多重关联并重复冲减；候选还会显示无余额消费。详见 `reports/09_phase4_red_team_review_2026-08-01.md`。
...
- 账单驱动重构：阶段 4 待修复退款来源真实性和一对一关联约束；重新红队审查通过后才能进入阶段 5
