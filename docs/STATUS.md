...
| 重构阶段 4 | ✅ 完成：退款候选匹配（原订单号/商户单号/同商户同金额/金额+90 天窗口打分）；人工关联（超额拒绝、审计、批次计数同步、跨期按原消费周期回写净成本）；安全批次撤销（未编辑自动入账可删，人工编辑/退款关联项阻塞列出，单事务回滚） | `docs/decisions/01_refactor_spec.md` §2.1/§2.3/§3.6/§5/§6/§7.4、`app/refunds/`、`app/revoke.py` |
| 当前产品面 | ✅ `/` 为 v2 迁移状态维护页；旧页面路由已下线，阶段 5 重建 | `app/routers/status.py` |
| 测试基线 | ✅ 直接 `pytest` 186 项通过（新增退款匹配/关联/跨期回写 8 项 + 撤销阻塞/幂等/跨批次 6 项） | `tests/test_refunds.py`、`tests/test_revoke.py` |
...
- 账单驱动重构：阶段 4（退款匹配、人工关联、跨期回写、安全批次撤销）已完成；下一步阶段 5（重建页面：概览/导入/待确认/流水/规则/批次 + 简易补账），待用户安排
