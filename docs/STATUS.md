...
- **最后更新**：2026-08-27
- **更新人**：用户反馈修复 + PRD 复审闭环（2026-08-27）
...
2. **正式库已完成重置并重新导入**（2026-08-01 15:00 重置，备份 `ledger.sqlite-20260801-150000.bak`、`ledger.sqlite-20260801-150040.bak`；2026-08-03 完成真实账单导入）；schema version 5（阶段 7 审计事件类型扩展）。
...
6. 真实账单导入已完成并验收（2026-08-14 复核通过，见 `reports/23_*`）；剩余 **1755 条**待确认（未匹配 1512、人际转账 108、中性资金流 66、退款 64、提现 5；红队复审回迁 28 条商家收款码误判，见 `reports/24_*`）需用户在 `/inbox` 逐批人工处理。处理完成前，正式统计为空是预期状态，不宣称"账已对平/已正式发布"。
7. PRD §3.3“低置信度”待确认原因未单独建模；目前由 `unmatched` + `observing_rule` 预填承担。若要做需先改规格并迁移。
8. 流水页 `list_entries_filtered(limit=500)` 会静默截断且无分页提示；待用户处理完 1755 条后可能不够，建议下轮做流水页分页。
9. PRD §3.4“退款调整”：简易补账可选 `refund` 类型，但该记录不参与统计、不关联原消费；需要用户确认口径后再改。
10. htmx 走 CDN，离线不可用（报告 25 已记录）；概览无月份切换（报告 23 已记录，规格未要求）。
11. `pending_count` 为全库口径，与“某月待确认”语义可能混淆（报告 24 已记录）。

## 进行中的工作

- 用户验收：真实账单已导入并通过验收复核；当前等待用户在 `/inbox` 处理 **1755 条**待确认（分类确认、退款关联、提现用途、人际/中性资金流定性），随后核对月度统计并生成用户验收报告。
- 本会话用户反馈修复已完成并写入 `docs/reports/28_*`；会话计划与发现记录在 `.planning/2026-08-27-user-feedback/`（不入 git）。
