...
- `PYTHONPATH=. pytest`：**270 passed**（基线 259 + 红队复审回归 6 + 同日 P3 跟进回归 5）。
- 正式库只读冒烟：`/`、`/inbox`、`/transactions`、`/rules`、`/imports` 均 200；`/?ym=abc` 200（修复生效）；待确认页含"方向"列与 direction 隐藏字段。

## 6. 遗留观察项与同日跟进

### 6.1 同日已修复的 P3（用户确认后跟进，2026-08-14）

- **P3 #2 旅游规则命中证据**：`match_rules` 不再过滤旅游规则；引擎对旅游规则（含遗留 active）一律按观察期预填入队并计 `hit_count`，绝不自动入账；`promote_rule` 与 `update_rule_status`（disabled→active 启用路径）均拒绝旅游规则，规则页提示明确原因。规格 §3.5 补充说明。
- **P3 #3 导入金额小数位**：`amount_to_cents` 对超 2 位小数整批解析拒绝（与手工补账口径一致），解析先于事务，坏金额永不落库。规格 §5 补充说明。
- **P3 #5 确认计数**：批量确认创建的观察规则按本组笔数累计 `confirm_count`，规则页「命中/确认」列不再恒为 0。规格 §3.5 补充说明。

回归测试：`tests/test_decisions.py` 新增 5 项（金额拒绝/2 位小数、旅游命中与禁止提升、遗留 active 旅游不自动入账、启用路径拒绝、确认计数）。

### 6.2 仍遗留的观察项（P3，非阻塞，不构成发布风险）

1. `pending_count` 在概览/导入历史为全库口径，与"某月待办"语义可能混淆（需用户定口径后再改）。
2. 导入批次处理中途失败（非解析阶段）无恢复入口，需重新上传（去重兜底）；属新功能设计，规格先行后再做。
